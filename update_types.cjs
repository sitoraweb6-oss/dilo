...
if (!file.includes('AccountMigration')) {
  // Let's add migrated status to UserProfile
  file = file.replace(/status: 'active' \| 'inactive' \| 'pending_approval' \| 'rejected';/g, "status: 'active' | 'inactive' | 'pending_approval' | 'rejected' | 'migrated' | 'pending_deletion';");
